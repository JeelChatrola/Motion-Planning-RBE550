...
class RRT(SamplingMethod):
    ...
    def get_neighbors(self, new_node):
        ...
        for node in self.samples:
            temp_d = self.robot.distance(node.config, new_node.config)
            if node != new_node and temp_d < 1.5 * self.kdtree_d:
    ...
    def rewire(self, new_node, neighbors):
        ...
        n_distances = [self.robot.distance(new_node.config, neighbor.config) for neighbor in neighbors]

        # Rewire the new node 
        # Compute the least cost as mentioned
        costs = []
        for i, d  in enumerate(n_distances):
            tpc = self.get_path_and_cost(self.start_node, neighbors[i])[1]
            costs.append(d + tpc)
        ...
        for i in indices:
            if not self.check_collision(new_node.config, neighbors[i].config):
                ...
                new_node.cost = n_distances[i]
                break

        # Rewire the neighbors
        for i, neighbor in enumerate(neighbors):
            npc = self.get_path_and_cost(self.start_node, new_node)[1] + n_distances[i]

            # if new cost is less than the previous cost no obstacles

            if self.get_path_and_cost(self.start_node, neighbor)[1] > npc and not self.check_collision(new_node.config, neighbor.config):
                neighbor.parent = new_node
                neighbor.cost = n_distances[i]
    ...
    def Informed_RRT_star(self):
        ...
        while len(self.samples) < self.n_configs:
            ...
            if self.goal_node.parent is not None:
                #print("GOAL Node:", self.goal_node.config)
                #print("GOAL Node parent:", self.goal_node.parent.config)
                c_best = self.get_path_and_cost(self.start_node, self.goal_node)[1]
                new_node = self.ext_inf_rrt(ext_d=self.kdtree_d, g_bias=0.05, c_b=c_best)
            else:
                except_count += 1
                new_node = self.extend(extension_d=self.kdtree_d, goal_bias=0.05)
    ...
    def get_sampling(self, goal_prob, best_path_len):
        '''
        Choose the goal or generate a random point in an ellipsoid defined by start, goal, 
        and the current best path length.

        Args:
            goal_prob: Probability of choosing the goal instead of a random point.
            best_path_len: Length of the current best path.
        ...
        if np.random.random() < goal_prob:
            ...
            x_c = [(self.start[i] + self.goal[i]) / 2 for i in range(n)]

            # Compute the rotation matrix from the ellipse to the world frame - C
            dir_v = [(self.start[i] - self.goal[i]) / c_min for i in range(n)]
            mat = np.array(dir_v + [0])
            ...
            r1 = best_path_len / 2
            r_o = np.sqrt(best_path_len ** 2 - c_min ** 2) / 2
            L = np.diag([r1] + [r_o] * n)
            ...
            x_rand = np.array(x_c) + CLx[:n]

            point_t = x_rand.tolist()
            point = []
            for i in point_t:
                point.append(i[0])
    ...
    def ext_inf_rrt(self, ext_d, g_bias, c_b):
        """Extend a new node from the current tree using informed sampling
        arguments:
            ext_d - the extension distance
            g_bias - the possibility of choosing the goal
                     instead of a random sample point
        ...
        new_pt = self.get_sampling(g_bias, c_b)
        # print(new_pt)

        nearest_node = self.get_nearest_node(new_pt)

        # Calculate new node location by extending the nearest node
        # compute the direction to move
        delta = np.zeros(self.robot.dof)
        for i in range(self.robot.dof):
            # position
            if (self.robot.limits[i][2] != "r"):
                delta[i] = new_pt[i] - nearest_node.config[i]
            # rotation
            else:
                # find the shortest angle
                delta[i] = angle_diff(
                    nearest_node.config[i], new_pt[i], absolute=False
                )
        # get unit vector
        ...
        new_cfg = (
            np.array(nearest_node.config) 
            + delta / np.linalg.norm(delta) * ext_d
        ).tolist()
        # wrap the angle if necessary
        for i in range(self.robot.dof):
            if self.robot.limits[i][2] == "r":
                new_cfg[i] = wrap_to_pi(new_cfg[i])

        # Check if the new configuration is valid
        if self.check_collision(new_cfg, nearest_node.config):
            return None

        # Create a new node
        new_node = Node(new_cfg)
        cost = self.robot.distance(new_cfg, nearest_node.config)
        # add the new node to the tree
        new_node.parent = nearest_node
        new_node.cost = cost
        self.samples.append(new_node)
