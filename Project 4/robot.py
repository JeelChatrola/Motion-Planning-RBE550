...
class Robot:
    ...
    def check_collision(
        ...
        configs_between = self.interpolate(config1, config2, 5)
    ...
    def check_collision_config(
        ...
        for robot_edge in robot_edges:
            robot_edges_n.append(((robot_edge[0][0], robot_edge[0][1]), (robot_edge[1][0], robot_edge[1][1])))

        if is_intersecting(robot_edges_n, obstacle_edges):
            return True
...
class OmnidirectionalRobot(Robot):
    ...
    def forward_kinematics(self, c):
        ...
        The last endpoint would be used for visualization of the sampling.
        arguments:
            c: [x, y, theta] of the rectangle

        return:
            e: 4 corner coordinates of the rectangle and its center
               [corner1, corner2, corner3, corner4, center]
        """
        assert len(c) == 3, "Configuration should be (x, y, theta)"
        e = np.zeros((5, 2))
        a, b, t = c
        w = self.width / 2
        h = self.height / 2
        e[0] = [a - w * np.sin(t) - h * np.cos(t),
                b - w * np.cos(t) + h * np.sin(t)]
        e[1] = [a + w * np.sin(t) - h * np.cos(t),
                b + w * np.cos(t) + h * np.sin(t)]
        e[2] = [a + w * np.sin(t) + h * np.cos(t),
                b + w * np.cos(t) - h * np.sin(t)]
        e[3] = [a - w * np.sin(t) + h * np.cos(t),
                b - w * np.cos(t) - h * np.sin(t)]
        e[4] = [a, b]  # Center of the robot
        return e
    ...
    def get_edges(self, config):
        ...
        edges = [(endpoints[i], endpoints[(i + 1) % 4]) for i in range(4)]
        return edges

    def distance(self, config1, config2):
        """Calculate the Euclidean distance between two configurations
        arguments:
            config1 - [x1, y1, theta1]
            config2 - [x2, y2, theta2]

        return:
            Euclidean distance in R^2 x S^1 space
        """
        x1, y1, theta1 = config1
        x2, y2, theta2 = config2
        x_diff = x1 - x2
        y_diff = y1 - y2
        angle_diff = utils.angle_diff(theta1, theta2)
        return np.sqrt(x_diff**2 + y_diff**2 + angle_diff**2)
    ...
    def interpolate(self, config1, config2, num=5):
        ...
        arguments:
            config1 - [x1, y1, theta1]
            config2 - [x2, y2, theta2]
            num - number of configurations to interpolate

        return:
            List with num configurations from linear interpolation in R^2 x S^1 space
        """
        x1, y1, theta1 = config1
        x2, y2, theta2 = config2
        ...
        interpolated_configs = list(zip(x_interp, y_interp, theta_interp))
...
class KinematicChain(Robot):
    ...
    def __init__(self, link_lengths, base=[0.1, 0.1]):
        ...
        # Limits in each degree of freedom
        # Assume all to be (-pi, pi)
        super().__init__(limits=[(-np.pi, np.pi, "r") for _ in range(self.num_joints)])

    def forward_kinematics(self, joint_angles):
        """Compute the joint coordinates given a configuration of joint angles.
        The last endpoint would be used for visualization of the sampling
        arguments:
            joint_angles: A list of joint angles in radians.

        return:
            joint_positions: A list of joint coordinates.
        """
        ...
        # Compute the end points of each joint based on the joint angles
        for i, joint_angle in enumerate(joint_angles):
            angle += joint_angle
            link_length = self.link_lengths[i]
            end_point = start_point + np.array([link_length * np.cos(angle), link_length * np.sin(angle)])
    ...
    def distance(self, config1, config2):
        ...
        euclidean_dist = np.linalg.norm(np.array(config1) - np.array(config2))
    ...
    def interpolate(self, config1, config2, num=10):
        ...
        for j in range(num + 1):
            beta = j / num
            new_config = [(1 - beta) * item1 + beta * item2 for item1, item2 in zip(config1, config2)]
            new_configs.append(new_config)
        return new_configs


    def is_l_intersecting(self, edge1, edge2, configuration):

        joint_positions = self.forward_kinematics(configuration)

        for i in range(len(joint_positions)):
            if i > 0:
                joint_positions[i] = joint_positions[i]

        line1_coords = list(edge1[0])
        a1, b1 = line1_coords
        a1_1, b1_1 = a1
        a2_1, b2_1 = b1
        line2_coords = list(edge2[0])
        a3, b3 = line2_coords
        a3_1, b3_1 = a3
        a4_1, b4_1 = b3
        line1_coords = [[a1_1, b1_1], [a2_1, b2_1]]
        line2_coords = [[a3_1, b3_1], [a4_1, b4_1]]
        ...
        m1 = (b2_1 - b1_1) / (a2_1 - a1_1)
        m2 = (b4_1 - b3_1) / (a4_1 - a3_1)

        if m1 == m2:
            return True

        intersection_point = line_intersection(line1_coords, line2_coords)

        if intersection_point:
            if list(intersection_point) not in joint_positions:
                return True
        return False

    

    def check_collision_config(self, configuration, map_boundaries, obstacles, obstacle_boundaries):
        # getting last point
        robot_end_point = self.forward_kinematics(configuration)[-1]

        # checking map_boundry
        if not is_in_polygon(robot_end_point, map_boundaries):
            return True

        # endpoint is in obstacle?
        for obstacle in obstacles:
            if is_in_polygon(robot_end_point, obstacle):
                return True

        # robot and obstacle collision
        robot_edges = self.get_edges(configuration)

        for robot_edge in robot_edges:
            robot_edge = [robot_edge]
            for obstacle_edge in obstacle_boundaries:
            ...
            for temp in robot_edges:
                temp = [temp]
                if temp != robot_edge:
                    if self.is_l_intersecting(temp, robot_edge, configuration):
                        return True
